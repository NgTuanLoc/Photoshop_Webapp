...
const App = () => {
  ...
  const uploadHandler = (e, image_name) => {
    e.preventDefault();
    const formData = new FormData();
    formData.append("file", imageFile, `${image_name}.png`);
  ...
  const objectDetectionHandler = () => {
    let t0 = performance.now();
    axios.get("http://localhost:5000/predict").then(function (response, data) {
      data = response.data;
      setPreviewImageUrl(data);
      let t1 = performance.now();
      console.log(
      ...
      <header className="App-header">
        <div className="App-upload">
          ...
          <div>
            <button
              type="submit"
              onClick={(event) => uploadHandler(event, "img_1")}
            >
              Submit
            </button>
          </div>
        ...
        <div className="App-upload">
          ...
          <div>
            <button
              type="submit"
              onClick={(event) => uploadHandler(event, "img_2")}
            >
              Submit
            </button>
          </div>
        ...
        <div>
          <button type="submit" onClick={() => objectDetectionHandler()}>
            Submit
          </button>
        </div>
      </header>
